...
async function getSpawnArgs(context: ExtensionContext) {
  ...
  if (!isDebug) {
    return {
      command: 'java',
      arguments: [ /* NYI: CLEANUP: remove enable-preview etc */`--enable-preview`, `--enable-native-access=ALL-UNNAMED`, `-Dfuzion.home=${context.extensionPath}/fuzion/build`, `-Dfile.encoding=UTF-8`, `-Xss${javaThreadStackSizeMB}m`, `-jar`, `./build/lsp.jar`, `-socket`, `--port=` + port],
      options: {
        env: {
          ...process.env,
          "PRECONDITIONS": "false",
